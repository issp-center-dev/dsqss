    ...
    def _get_info(self, inputfile):
        ...
        for key in tmp_dict.keys():
            ...
            if key in self.prm.param_dict:
                ...
                invalid = False
            if invalid:
                warnings.warn("Unknown keyword \" " + key + "\" is found (ignored).", Warning)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Generate input files for dsqss',
        add_help=True,
    )

    parser.add_argument('-g', '--generator-dir', dest='gendir',
    ...
    parser.add_argument('--hamfile', dest='hfile',
                        default='hamiltonian.xml',
                        help='Hamiltonian file',
                        )
    ...
    if args.input is None:
        raise RuntimeError('Neither --input (read from file) nor --stdin (read from stdin) option is used.')
    calcinfo = info(args)
    calcinfo.make_xml()
