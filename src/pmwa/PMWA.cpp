...
double Dla::PMWA() {
  ...
  Configuration CS(&MC, &N, sp.nmax, &LT, &P, &PR, IMAX, WMAX, Eventfile_old,
                   &MR, (bool)cb, outfile);
...
void Dla::NameOutfiles() {
  ...
  Eventfile += outfile;
  Eventfile +=
      std::string("_rank") + tostring(PR.my_rank) + std::string(".dat");
...
void Dla::set_Parallel() {
  ...
  PR.Npara =
      PR.p_num / PR.NtNs;  // the number of seeds for trivial parallelization.
  ...
  if (PR.Rpara > 0) {
    PR.nr =
        PR.np %
        PR.Rpara;  // a random potential number (one of trivial parallelization)
    PR.nq = PR.np /
