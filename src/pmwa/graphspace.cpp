...
GraphSpace::GraphSpace(Size *N, int m_nmax, Lattice *m_LT, Probability *m_P, Parallel *m_PR, long long m_IMAX,
                       long long m_WMAX, std::string const& m_Eventfile_old, My_rdm *MR, int cb, std::string const& m_outfile) {
  IMAX = m_IMAX;
  WMAX = m_WMAX;
  NMIN = G_NMIN;
  outfile = m_outfile;
...
void GraphSpace::initialev(std::string const& Eventfile_old, My_rdm *MR, int cb, double oldB) {
  ev.resize(IMAX);
  ev.clear();
  ...
  if(PR->FlgAnneal || PR->FlgRestart) {
    ...
    if (fin) {
      cout << "rank " << my_rank << ": checkpoint file" << Eventfile_old << " found, resume this calculation." << endl;
      ...
      if (PR->FlgAnneal) {
          if (fabs(tmp_B_old-BetaOldOrg) < pow(10.0, -12)) {
              ...
              if(PR->my_rank==0) {
                  cerr << "oldB = "<<BetaOldOrg <<" is not same the beta ="<<tmp_B_old<< " written in the input file"<< endl;
              }
              fin.close();
              MPI_Finalize();
              exit(1);
            ...
            if (PR->FlgRestart){
                if (fabs(tmp_B_old - BetaOrg) < pow(10.0, -12)) {
                    ...
                    if(PR->my_rank==0) {
                        cerr << "B = "<< BetaOrg <<" is not same the beta ="<<tmp_B_old<< " written in the input file"<< endl;
                    }
                    fin.close();
                    MPI_Finalize();
                    exit(1);
      ...
      if(PR->my_rank==0) {
        cerr << "rank " << my_rank << ":no evfile found, start a new calculation" << endl;
      }
      MPI_Finalize();
      exit(1);
    }
    ...
    if (cb == 1)
      for (int i = 0; i < V; i++)
        world[i].p = worldB[i].p = ((i % PR->x + (int) (i / PR->x)) % 2 == 0) ? 1 : 0;
    else if (cb == 2)
      for (int i = 0; i < V; i++)
...
void GraphSpace::Assign_TwoSiteVertex(My_rdm *MR) {
  ...
  for (int d = 0; d < LT->Pd; d++) {
    ...
    MPI_Sendrecv(&(NewVertex_th0[d]), 1, MPI_INT, PR->left[d], 0, &(NewVertex_th1[d]), 1, MPI_INT, PR->right[d], 0,
                 MPI_COMM_WORLD, &status);
  }

  for (int d = 0; d < LT->Pd; d++) {
    MPI_Sendrecv(&(BoxSpace_t_th0[d][0]), NewVertex_th0[d], MPI_DOUBLE, PR->left[d], 0, &(BoxSpace_t_th1[d][0]),
                 NewVertex_th1[d], MPI_DOUBLE, PR->right[d], 0, MPI_COMM_WORLD, &status);
    MPI_Sendrecv(&(BoxSpace_p_th0[d][0]), NewVertex_th0[d], MPI_SHORT, PR->left[d], 0, &(BoxSpace_p_th1[d][0]),
                 NewVertex_th1[d], MPI_SHORT, PR->right[d], 0, MPI_COMM_WORLD, &status);
  }
  ///////////////////////////////////////////////////////////////////////////////////////////////
  ///////////////////////////////////////////////////////////////////////////////////////////////
  ...
  int ntmax;
  for (int d = 0; d < LT->Pd; d++) {
    MPI_Sendrecv(newt[d], LT->Fx[d], MPI_INT, PR->right[d], 0, newt_th1[d], LT->Fx[d], MPI_INT, PR->left[d], 0,
                 MPI_COMM_WORLD, &status);

    ntmax = 0;
    for (int i = 0; i < LT->Fx[d]; i++)
      ntmax = max(ntmax, newt[d][i]);
    NewVertex_th0[d] = ntmax * LT->Fx[d];

    ntmax = 0;
    for (int i = 0; i < LT->Fx[d]; i++)
      ntmax = max(ntmax, newt_th1[d][i]);
    NewVertex_th1[d] = ntmax * LT->Fx[d];

    MPI_Sendrecv(&(BoxSpace_t_th0[d][0]), NewVertex_th0[d], MPI_DOUBLE, PR->right[d], 0, &(BoxSpace_t_th1[d][0]),
                 NewVertex_th1[d], MPI_DOUBLE, PR->left[d], 0, MPI_COMM_WORLD, &status);
...
void GraphSpace::insert(Vertex *vl, Vertex *vr, short new_type, double new_time, int xl, int xr, int pl, int pr,
                        int d) {
  //Left
  if (VertexPool.empty())
...
void GraphSpace::parity_check(My_rdm *MR, int py, double *x, int &j, double topt, double bottomt) {
  ...
  if (j != 0) {
    if (Il <= (double)(j + 1) * NMIN) {
      if (PR->my_rank == 0)
        cout << "Il is too small: Il=" << Il << ", topt=" << topt << ", bottomt=" << bottomt << endl;
      j = py % 2;
      if (Il <= (double)(j + 1) * NMIN) {
...
void GraphSpace::SpatialDomainBoundary(My_rdm *MR) {
  ...
  for (int d = 0; d < LT->Pd; d++) {
    ...
    MPI_Sendrecv(&(NewVertex_th0[d]), 1, MPI_INT, PR->left[d], 0, &(NewVertex_th1[d]), 1, MPI_INT, PR->right[d], 0,
                 MPI_COMM_WORLD, &status);
    MPI_Sendrecv(&(BoxSpace_py_th0[0]), NewVertex_th0[d], MPI_SHORT, PR->left[d], 3, &(BoxSpace_py_th1[0]),
                 NewVertex_th1[d], MPI_SHORT, PR->right[d], 3, MPI_COMM_WORLD, &status);
    MPI_Sendrecv(&(BoxSpace_p_th0[d][0]), NewVertex_th0[d], MPI_SHORT, PR->left[d], 1, &(BoxSpace_p_th1[d][0]),
                 NewVertex_th1[d], MPI_SHORT, PR->right[d], 1, MPI_COMM_WORLD, &status);
    MPI_Sendrecv(&(BoxSpace_type_th0[0]), NewVertex_th0[d], MPI_SHORT, PR->left[d], 4, &(BoxSpace_type_th1[0]),
                 NewVertex_th1[d], MPI_SHORT, PR->right[d], 4, MPI_COMM_WORLD, &status);
    MPI_Sendrecv(&(BoxSpace_t_th0[d][0]), NewVertex_th0[d], MPI_DOUBLE, PR->left[d], 2, &(BoxSpace_t_th1[d][0]),
                 NewVertex_th1[d], MPI_DOUBLE, PR->right[d], 2, MPI_COMM_WORLD, &status);
    ...
    MPI_Sendrecv(&(BoxSpace_p_th1[d][0]), NewVertex_th1[d], MPI_SHORT, PR->right[d], 0, &(BoxSpace_p_th0[d][0]),
                 NewVertex_th0[d], MPI_SHORT, PR->left[d], 0, MPI_COMM_WORLD, &status);
    MPI_Sendrecv(&(BoxSpace_py_th1[0]), NewVertex_th1[d], MPI_SHORT, PR->right[d], 0, &(BoxSpace_py_th0[0]),
                 NewVertex_th0[d], MPI_SHORT, PR->left[d], 0, MPI_COMM_WORLD, &status);
    MPI_Sendrecv(&(BoxSpace_type_th1[0]), NewVertex_th1[d], MPI_SHORT, PR->right[d], 0, &(BoxSpace_type_th0[0]),
                 NewVertex_th0[d], MPI_SHORT, PR->left[d], 0, MPI_COMM_WORLD, &status);
...
void GraphSpace::OldBox() {
  ...
  MPI_Sendrecv(&(p1_box[0]), V, MPI_SHORT, PR->upper, 0, &(p0_box[0]), V, MPI_SHORT, PR->lower, 0, MPI_COMM_WORLD,
               &status);
  MPI_Sendrecv(&(i1_box[0]), V, MPI_DOUBLE, PR->upper, 0, &(i0_box[0]), V, MPI_DOUBLE, PR->lower, 0, MPI_COMM_WORLD,
               &status);
}
...
void GraphSpace::NewBox() {
  ...
  MPI_Sendrecv(&(p0_box[0]), V, MPI_SHORT, PR->lower, 0, &(p1_box[0]), V, MPI_SHORT, PR->upper, 0, MPI_COMM_WORLD,
               &status);

  for (int site = 0; site < V; site++)
    worldB[site].dir = p1_box[site];
}
